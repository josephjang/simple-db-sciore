...
public class Predicate {
    ...
    public boolean isSatisfied(Scan s) {
        for (Term t : terms) if (!t.isSatisfied(s)) return false;
        return true;
    }
    ...
    public int reductionFactor(Plan p) {
        int factor = 1;
        for (Term t : terms) factor *= t.reductionFactor(p);
        return factor;
    }
    ...
    public Predicate selectSubPred(Schema sch) {
        ...
        for (Term t : terms) if (t.appliesTo(sch)) result.terms.add(t);
    ...
    public Predicate joinSubPred(Schema sch1, Schema sch2) {
        ...
        newsch.addAll(sch1);
        newsch.addAll(sch2);
        for (Term t : terms) if (!t.appliesTo(sch1) && !t.appliesTo(sch2) && t.appliesTo(newsch)) result.terms.add(t);
        if (result.terms.size() == 0) return null;
        else return result;
    }
