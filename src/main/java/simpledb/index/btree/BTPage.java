...
public class BTPage {
    ...
    public void format(BlockId blk, int flag) {
        ...
        for (int pos = 2 * Integer.BYTES; pos + recsize <= tx.blockSize(); pos += recsize) makeDefaultRecord(blk, pos);
    }
    ...
    public void delete(int slot) {
        for (int i = slot + 1; i < getNumRecs(); i++) copyRecord(i, i - 1);
    ...
    private Constant getVal(int slot, String fldname) {
        ...
        if (type == INTEGER) return new Constant(getInt(slot, fldname));
        else return new Constant(getString(slot, fldname));
    }
    ...
    private void insert(int slot) {
        for (int i = getNumRecs(); i > slot; i--) copyRecord(i - 1, i);
        setNumRecs(getNumRecs() + 1);
    }

    private void copyRecord(int from, int to) {
        Schema sch = layout.schema();
        for (String fldname : sch.fields()) setVal(to, fldname, getVal(from, fldname));
    }
    ...
    private void transferRecs(int slot, BTPage dest) {
        ...
        while (slot < getNumRecs()) {
            ...
            Schema sch = layout.schema();
            for (String fldname : sch.fields()) dest.setVal(destslot, fldname, getVal(slot, fldname));
            delete(slot);
            destslot++;
        }
    }
