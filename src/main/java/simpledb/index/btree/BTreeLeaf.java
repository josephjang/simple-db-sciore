...
public class BTreeLeaf {
    ...
    public boolean next() {
        currentslot++;
        if (currentslot >= contents.getNumRecs()) return tryOverflow();
        else if (contents.getDataVal(currentslot).equals(searchkey)) return true;
        else return tryOverflow();
    }
    ...
    public DirEntry insert(RID datarid) {
        ...
        if (lastkey.equals(firstkey)) {
            ...
            if (splitkey.equals(firstkey)) {
                ...
                while (contents.getDataVal(splitpos).equals(splitkey)) splitpos++;
                splitkey = contents.getDataVal(splitpos);
            } else {
                // move left, looking for first entry having that key
                while (contents.getDataVal(splitpos - 1).equals(splitkey)) splitpos--;
            }
    ...
    private boolean tryOverflow() {
        ...
        if (!searchkey.equals(firstkey) || flag < 0) return false;
        contents.close();
